...
minimum_columns_from_excel_extract = [
    "Activity Date",
    "Activity Type",
    "Elapsed Time",
    "Moving Time",
...
full_columns_from_excel_extract = [
    "Activity ID",
    "Activity Date",
    "Activity Name",
    "Activity Type",
...
allowable_activities = ["Run", "Ride", "Swim", "Walk", "Basket", "HIIT"]

colours_of_activities = ["Orange", "Green", "Purple", "Blue", "White", "Gray"]
...
final_order_of_columns_for_table_display = [
    "Activity ID",
    "Activity Name",
    "Activity Type",
    "Activity Date",
...
@st.cache_data
def load_data(file, units=("km", "m", "kph")):
    ...
    if set(minimum_columns_from_excel_extract).issubset(data.columns):
        ...
        data["Ratio of Move to Total Time"] = (
            data["Moving Time (hours)"] / data["Elapsed Time (hours)"]
        )
        ...
        if units[0] == "km":
            ...
            data["Average Speed"] = round(data["Distance"] / (data["Moving Time (hours)"] / 60), 2)
            data[elevation_columns] = (
                data[elevation_columns].apply(lambda x: x * metre_foot_conv).round(2)
            )
        data["Elapsed Time (hours)"] = round((data["Elapsed Time (hours)"]) / 60, 2)
        data["Moving Time (hours)"] = round((data["Moving Time (hours)"]) / 60, 2)
        ...
        st.error(
            "Error reading .csv extract. Please ensure it has the correct column names and allowable values. If issue persists, contact details in side menu.",
            icon="🚨",
        )
        sys.exit()
...
def slider_filter(message, df):
    ...
    if min_value < max_value:
        dates_selection = st.slider(
            ...
            min_value=min(df["Activity Date"].dt.date),
            max_value=max(df["Activity Date"].dt.date),
            value=(min(df["Activity Date"].dt.date), max(df["Activity Date"].dt.date)),
...
def select_slider_filter(message, df):
    ...
    if df.empty:
        ...
        st.stop()
    if min_value < max_value:
        dates_selection = st.select_slider("%s" % (message), options=options)
...
def format_metric_values(df, merged_df, activity_type, metric, units):
    ...
    formatted_value = "{:,.2f} ".format(
        df.at[df[df["Activity Type"] == activity_type].index[0], metric]
    )
    formatted_change = "{:.2f}%".format(
        merged_df.at[merged_df[merged_df["Activity Type"] == activity_type].index[0], metric]
    )
...
def create_metrics(
    df, merged_df, activity_type, first_column, second_column, third_column, four_column, units
):
    st.markdown(
        """
        <style>
    ...
    formatted_activity_count_value = "{:,} ".format(
        df.at[df[df["Activity Type"] == activity_type].index[0], "Activity Date"]
    )
    formatted_activity_count_gain = "{:.2f}%".format(
        (
            merged_df.at[
                merged_df[merged_df["Activity Type"] == activity_type].index[0], "Activity Date"
            ]
        )
    )
    formatted_hour_value = df.at[
        df[df["Activity Type"] == activity_type].index[0], "Moving Time (hours)"
    ]
    formatted_hour_gain = "{:.2f}%".format(
        (
            merged_df.at[
                merged_df[merged_df["Activity Type"] == activity_type].index[0],
                "Moving Time (hours)",
            ]
        )
    )
    formatted_distance_value, formatted_distance_change = format_metric_values(
        df, merged_df, activity_type, "Distance", units[0]
    )
    formatted_elevation_value, formatted_elevation_change = format_metric_values(
        df, merged_df, activity_type, "Elevation Gain", units[1]
    )
    col0.metric("", activity_type, "")
    col1.metric(
        "Total Number of Activities", formatted_activity_count_value, formatted_activity_count_gain
    )
...
def create_area_chart(df, y_axis, units):
    ...
    chart = (
        ...
        .encode(
            x="Activity Date:T",
            y=y,
            color=alt.Color("Activity Type:N", legend=None).scale(
                domain=allowable_activities, range=colours_of_activities
            ),
            tooltip=[
                ...
                alt.Tooltip(
                    "Elevation Gain:Q", format=",.2f", title=f"Elevation Gain ({units[1]})"
                ),
            ],
        )
        .interactive()
...
def create_scatter_graph(df, units):
    ...
    chart = (
        ...
        .encode(
            ...
            color=alt.Color("Activity Type:N", legend=None, sort=allowable_activities).scale(
                domain=allowable_activities, range=colours_of_activities
            ),
            tooltip=[
                ...
                alt.Tooltip(
                    "Elevation High:Q", format=",.2f", title=f"Elevation High ({units[1]})"
                ),
            ],
            size=alt.condition(param_checkbox, "Elevation Gain:Q", alt.value(25), legend=None),
        )
...
def create_average_speed_line_chart(df, units):
    ...
    df_copy = (
        df_copy.groupby(["Date", "Activity Type"]).agg({"Average Speed": "mean"}).reset_index()
    )
    ...
    merged_df["Average Speed"] = merged_df.groupby("Activity Type")["Average Speed"].transform(
...
def create_mark_bar_weighted_average(df, units):
    ...
    chart = (
        ...
        .encode(
            ...
            color=alt.Color("Period of Day:N", legend=None).scale(
                domain=periods_of_day, range=colours_of_periods_of_day
            ),
            row=alt.Row("Activity Type:N", sort=allowable_activities),
            tooltip=[
                alt.Tooltip("Period of Day:N"),
                alt.Tooltip("Activity Type:N"),
                alt.Tooltip(
                    "Weighted Avg Speed:Q", format=",.4f", title=f"Weighted Avg Speed ({units[2]})"
                ),
            ],
        )
        .interactive()
...
def create_regression_scatter_graph(df, metric, units):
    if df.empty:
        st.error(
            "No data to display. Ensure there is at least 2 activities within selected filters.",
            icon="🚨",
        )
        st.stop()
    ...
    scatter_graph = (
        ...
        .encode(
            ...
            y=y,
            color=alt.Color(
                "Activity Type:N", legend={"orient": "top"}, sort=allowable_activities
            ).scale(domain=allowable_activities, range=colours_of_activities),
            tooltip=[
                alt.Tooltip("Activity Type:N"),
                alt.Tooltip("Activity Date:T"),
                alt.Tooltip(
                    "Cumulative Distance:Q",
                    format=",.0f",
                    title=f"Cumulative Distance ({units[0]})",
                ),
                alt.Tooltip("Cumulative Moving Time:N"),
                alt.Tooltip(
                    "Cumulative Elevation Gain:Q",
                    format=",.0f",
                    title=f"Cumulative Elevation Gain ({units[1]})",
                ),
            ],
        )
        .interactive()
...
def create_regression(
    ...
    if number_of_activities < 2:
        return
    X = (
        data["Activity Date"].astype(np.int64).values.reshape(-1, 1)
    ...
    chart = (
        alt.Chart(future_df)
        .mark_line(color=color_of_trendline, strokeDash=[1, 2])
        .encode(
            x="Activity Date:T",
    ...
    for vertical_date in list_of_vertical_lines:
        vertical_line = (
            alt.Chart(pd.DataFrame({"vertical_date": [vertical_date]}))
            .mark_rule(strokeDash=[10, 5], color="red")
            .encode(x=alt.X("vertical_date:T", title=""))
        )
        vertical_date = vertical_date.strftime("%Y-%m-%d")
        predicted_value = np.round(
            future_df.loc[future_df["Activity Date"] == vertical_date, f"Predicted {metric}"].iloc[
                0
            ]
        ).astype(int)
        ...
        label = activity_type + ":\n" + str(predicted_value)
        value_annotation = (
            alt.Chart(pd.DataFrame({"vertical_date": [vertical_date], "label": label}))
            .mark_text(dx=0, dy=(index), color="black")
            .encode(x="vertical_date:T", text="label")
        )
        if index == 0:
            date_annotation = (
                alt.Chart(
                    pd.DataFrame({"vertical_date": [vertical_date], "label": [vertical_date]})
                )
                .mark_text(dx=0, dy=-15, color="black")
                .encode(x="vertical_date:T", text="label")
            )
...
def create_scatter_graph_and_regression(df, metric, units, predictor_1, predictor_2, end_date):
    try:
        ...
        for index, activity_type in enumerate(df["Activity Type"].unique()):
            combined_chart = create_regression(
                df,
                combined_chart,
                metric,
                activity_type,
        ...
        st.error(
            "Adjust the configuration so your predictor dates are within the range of the regression line",
            icon="🚨",
        )
    return (
        combined_chart,
...
def write_statistics_of_regression(
    activity_type,
    r_squared_values,
    mae_values,
    numbers_of_activities,
    ...
    try:
        ...
        for key in keys:
            st.write(key, ": ", str(predicted_values_activity_type[activity_type][key]), units)
        st.caption(
            f"Number of activities in model calculation: __{numbers_of_activities[activity_type]}__"
        )
        st.caption(f"The model captures about __{r_squared}%__ of the patterns in the data.")
        st.caption(
            f"On average, the predictions __are off by {mae_values[activity_type]} {units}__ compared to the actual values."
        )
    except Exception as e:
...
def create_columns_for_statistics(
    df,
    predicted_values_activity_type,
    r_squared_values,
    mae_values,
    ...
    if number_of_activity_types == 1:
        write_statistics_of_regression(
            activity_types[0],
            r_squared_values,
            mae_values,
            numbers_of_activities,
